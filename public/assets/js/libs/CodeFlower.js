var CodeFlower = function(selector, w, h) {
  ...
  this.force = d3.layout.force()
    ...
    .charge(function(d) { 
      return d._children ? -d.size / 10 : -30; })
    .linkDistance(function(d) {
      return d.target.children ? 90 : 25; 
    })
    .linkDistance(function(d) {
      return d.target.power ? Math.abs(d.target.power) : 10;
    })
    .size([h, w]);
};
...
CodeFlower.prototype.update = function(json) {
  ...
  this.node.enter().append('svg:circle')
    ...
    .attr("r", function(d) { return d.children ? 7.5 : Math.pow(d.size, 50) || 1; })
    .style("fill", function color(d) {

      if (d.name === "Probe") {
        return "hsl(100,90%,70%)";
      }
      else if (d.children.length === 0) {
        return "hsl(50,90%,70%)"
      } else if (d.children.length > 0 && d.index > 0) {
        return "hsl(215,90%,70%)"
      } 
    })
